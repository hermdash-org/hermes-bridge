...
async def connect():
    ...
    """
    try:
        # Use the same approach as CLI: remove existing tokens and probe
        ...
        if "higgsfield" not in servers:
            raise HTTPException(
                status_code=404,
                detail="Higgsfield not configured in mcp_servers"
            )
        
        server_config = servers["higgsfield"]
        
        # Clear existing OAuth state (force re-auth)
        try:
            mgr = get_manager()
            mgr.remove("higgsfield")
        except Exception as exc:
            logger.warning(f"Could not clear existing OAuth state: {exc}")
        
        # Trigger OAuth flow by probing the server
        # This will open browser and wait for callback
        from hermes_cli.mcp_config import _probe_single_server
        
        tools = _probe_single_server("higgsfield", server_config)
        
        return {
            "message": f"Successfully authenticated. {len(tools)} tools available.",
            "success": True,
