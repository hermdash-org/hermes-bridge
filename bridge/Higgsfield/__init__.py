...
async def connect():
    ...
    """
    try:
        # Generate OAuth URL using MCP OAuth module
        ...
        oauth_config = {
            ...
            "redirect_port": 0  # Auto-pick free port
        }
        
        # This will trigger the OAuth flow
        # The build_oauth_auth function handles the callback server
        auth_provider = build_oauth_auth(
        ...
        if not auth_provider:
            raise HTTPException(
                status_code=500,
                detail="OAuth not available. MCP SDK may not be installed."
            )
        
        # The OAuth flow is now active
        # User needs to complete it in browser
        return {
            "message": "OAuth flow started. Check your browser to authenticate.",
            "oauth_url": None  # URL is opened automatically by MCP SDK
        }
